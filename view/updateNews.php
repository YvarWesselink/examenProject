    ...
    <form method="post" class="opdrachtIndienenForm" >
        <div>
            ...
            if (isset($_POST['updateNews'])) {
                $errormsg = updateNews::checkExcersise($_POST);

                if (!array_filter($errormsg)) {
                    updateNews::UploadExersise($_POST);
                }
            }
            updateNews::showFields($errormsg);
            ?>
            <!-- Dit is het form voor de gegevens van het bedrijf. -->
            <input type="submit" class="sendExcersiseBtn" name="updateNews" value="Nieuws updaten">
    </form>
</div>
</html>
